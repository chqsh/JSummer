...
class CheckMDFile {
    ...
    protected class BOM {
        ...
        public boolean hasError() {
            return (this.nBytes < 0);
        }

        public int getErrorCode() {
            return (this.nBytes < 0) ? this.nBytes : 0;
        }

        public String getErrorMsg() {
            return (this.nBytes < 0) ? this.encoding : "";
        }
    ...
    protected boolean parseFile() {
        ...
        try {
            ...
            int lineErrors = 0;
            if (bom.exists()) {
                br.skip(1L); // skip one char (One UTF-8 char is 3 bytes.)
            }
            while (true) {
                lineNr++;
                String l = br.readLine();
