...
class XWindow {
	...
	protected void _startx() {
		...
		helpmenue.setText("&Help");
		final Menu helpMenu = new Menu(helpmenue);
		helpmenue.setMenu(helpMenu);
		// done menue entries

		/* Don't use next methods, as these op will conflect with default action of "&C" ALT+hot_char */
		/*
		hashmenue.setAccelerator(SWT.ALT | 'a');
		...
		*/
		...
		newFile.setText("Add file(s)\tCTRL+O");
		newFile.setAccelerator(SWT.CTRL + 'O');
		...
		newDir.setText("Add directory\tCTRL+D");
		newDir.setAccelerator(SWT.CTRL + 'D');
		...
		addCheckFile.setText("Open check-file\tALT+O");
		addCheckFile.setAccelerator(SWT.ALT + 'O');
		...
		this.menuSaveFile.setText("Save hash ...\tCTRL+S");
		this.menuSaveFile.setAccelerator(SWT.CTRL + 'S');
		...
		try {
			...
			if (iconURL != null) {
				final InputStream s = iconURL.openStream();
				final Image icon = new Image(this.xdisplay, s);
				this.menuSaveFile.setImage(icon);
		...
		clear.setText("Clear table ...\tCTRL+N");
		clear.setAccelerator(SWT.CTRL + 'N');
		...
		selectHash.setToolTipText("Select the hash-algorithm ... (CTRL+M)");
		...
		selectHash.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				showSelectHashMenu(toolBar, selectHash, selectHashMenue);
			}
		});

		// Global Accelerator
		xdisplay.addFilter(SWT.KeyDown, new Listener() {
			public void handleEvent(Event event) {
				// System.out.printf("tyep %d, keyCode %d, character '%c', stateMask %d%n",
				// 	event.type, event.keyCode, event.character, event.stateMask);
				if (event.type == SWT.KeyDown) {
					// if (event.stateMask == SWT.ALT && event.keyCode == 97) {
					// 	/* ALT+A (KEY_A is 97) */
					// 	System.out.println("ALT+A");
					// }
					if (event.stateMask == SWT.CTRL && event.keyCode == 109) {
						/* CTRL+M (KEY_M is 109) */
						showSelectHashMenu(toolBar, selectHash, selectHashMenue);
					}
				}
		...
		final MenuItem menuEncoding = new MenuItem(configMenu, SWT.CASCADE);
		menuEncoding.setText("check-file encoding ...");
		final Menu encodingMenue = new Menu(this.xshell, SWT.DROP_DOWN);
		menuEncoding.setMenu(encodingMenue);
		int n = 0;
		xencoding = new XEncoding(this, encodingMenue);
		List<String> encodingText = xencoding.getCharsetName();
		for (String enc : encodingText) {
			final MenuItem eit = new MenuItem(encodingMenue, SWT.RADIO);
			eit.setText(enc);
			if (n == 0) eit.setSelection(true);
			eit.addSelectionListener(xencoding);
			n++;
		}
		...
		aboutmenu.setText("about ...\tF1");
		aboutmenu.setAccelerator(SWT.F1);
		// aboutmenu.setImage(xdisplay.getSystemImage(SWT.ICON_INFORMATION));
		final ToolItem abouttool = new ToolItem(toolBar, SWT.PUSH);
		abouttool.setToolTipText("about ... (F1)");
		try {
		...
		final String tableToolTipText0 = "double-click on next one item to view details ...";
		final String tableToolTipText1 = "double-click on next one item to view details and\nto copy hash to clipboard ...";
		column0.setToolTipText(tableToolTipText0);
		column2.setToolTipText(tableToolTipText0);
		column1.setToolTipText(tableToolTipText1);
		...
		comp.addControlListener(new ControlAdapter() {
			public void controlResized(ControlEvent e) {
				...
				XWindow.this.o.debug(
						"controlResized :: area.width=" + area.width + "; c0width=" + c0width + "; c1width=" + c1width,
						XWindow.this.classDebugLevel);
		...
		target.addDropListener(
				new DropTargetAdapter() {
					// DropTargetListener is a abstract interface. So, you MUST override every
					// method.
					 ...
					 * // Refer To:
					 * //
					 * https://github.com/blizzy78/blizzys-backup/blob/master/de.blizzy.backup/src/
					 * de/blizzy/backup/settings/SettingsDialog.java
					 * 
					 * System.out.
					 * printf("%s, {detail 0x%X, feedback 0x%X, operations 0x%X}\n\t currentDataType is %s\n"
					 * ,
					 * event.toString(), event.detail, event.feedback, event.operations,
					 * event.currentDataType.toString());
					 * 
					 * if (!FileTransfer.getInstance().isSupportedType(event.currentDataType)) {
					 * event.detail = DND.DROP_NONE; // DO NOT accept drop operation. The "drop()"
					 * will NOT be called!
					 * }
					 * }
					 */
		...
		// adjust hash-selection and title by config
		Enumeration<HashSumInfo> hie2 = this.config.containerHashSumInfo.elements();
		while (hie2.hasMoreElements()) {
			HashSumInfo hsi = hie2.nextElement();
		...
		// adjust check-file encoding
		{
			String cfgEncoding = this.config.getCheckMDFileEncoding();
			if (!cfgEncoding.isEmpty()) {
				xencoding.setSelection(cfgEncoding);
			}
		}
	 ...
	 * long tomb = 1024;
	 * float used =
	 * (Runtime.getRuntime().totalMemory()-Runtime.getRuntime().freeMemory())/tomb;
	...
	private static String totalKB = "", usedKB = "";
	private static int total0 = 0, used0 = 0;
	...
	protected String getMemStats() {
		...
		float total = Runtime.getRuntime().totalMemory() / tomb;
		if ((int) used != used0) {
			usedKB = nf.format(used);
			used0 = (int) used;
		}
		if ((int) total != total0) {
			totalKB = nf.format(total).intern();
			total0 = (int) total;
		}
	...
	protected void setMemstats() {
		this.xdisplay.asyncExec(new Runnable() {
			public void run() {
				...
				XWindow.this.memlabel.setText(getMemStats());
		...
		this.xdisplay.asyncExec(new Runnable() {
			public void run() {
				...
				if (useRelPath) {
					...
					/* [20260427] Maybe logic confusion.
	...
	protected synchronized void setUpdateName(boolean updateName) {
		this.o.debug(this.toString() + "::setUpdateName():" + updateName + " (was: " + this.updateName + ")",
				this.classDebugLevel);
	...
	protected String getEncodingMenuSelection() {
		if (xencoding == null)
			return "";
		else
			return xencoding.getSelection();
	}
	...
	protected static void showSelectHashMenu(final ToolBar toolBar,
		final ToolItem selectHash, final Menu selectHashMenu) {
		Rectangle rect = selectHash.getBounds();
		Point pt = new Point(rect.x, rect.y + rect.height);
		pt = toolBar.toDisplay(pt);
		selectHashMenu.setLocation(pt.x, pt.y);
		selectHashMenu.setVisible(true);
	}
